...
describe("PEDIDO_STAGES", () => {
  it("possui as 9 colunas do fluxo novo na ordem correta", () => {
...
describe("ALLOWED_FORWARD — fluxo linear", () => {
  it("avança etapa a etapa", () => {
    ...
    expect(isTransitionAllowed("em_producao", "em_transito")).toBe(true);
    expect(isTransitionAllowed("em_transito", "pronto")).toBe(true);
    expect(isTransitionAllowed("pronto", "entrega")).toBe(true);
    expect(isTransitionAllowed("entrega", "faturado_em_rota")).toBe(true);
  ...
  it("pular etapas é recusado", () => {
    ...
    expect(isTransitionAllowed("em_producao", "pronto")).toBe(false);
    expect(isTransitionAllowed("pronto", "faturado_em_rota")).toBe(false);
  });
