...
-- 1) Nova coluna: dias limite sem PROPOSTA (calibrado B2B), padrão 14
ALTER TABLE public.xerife_config
  ADD COLUMN IF NOT EXISTS placar_dias_sem_proposta_limite integer NOT NULL DEFAULT 14;

-- 2) Bônus escalonado de meta (mantém peso_meta_batida como referência do 100%)
-- Coeficientes aplicados sobre placar_peso_meta_batida:
-- 50% → 0.25x · 80% → 0.5x · 100% → 1x · 120% → 1.5x

-- 3) Histórico mensal de metas (snapshot congelado no fim do mês)
CREATE TABLE IF NOT EXISTS public.vendedor_metas_historico (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  ano int NOT NULL,
  mes int NOT NULL,
  meta_valor numeric(14,2) NOT NULL DEFAULT 0,
  ganhos_valor numeric(14,2) NOT NULL DEFAULT 0,
  ganhos_qtd int NOT NULL DEFAULT 0,
  atingido_pct numeric(6,2) NOT NULL DEFAULT 0,
  bateu boolean NOT NULL DEFAULT false,
  snapshot_at timestamptz NOT NULL DEFAULT now(),
  UNIQUE(user_id, ano, mes)
);

GRANT SELECT, INSERT, UPDATE ON public.vendedor_metas_historico TO authenticated;
GRANT ALL ON public.vendedor_metas_historico TO service_role;

ALTER TABLE public.vendedor_metas_historico ENABLE ROW LEVEL SECURITY;

CREATE POLICY "self view historico" ON public.vendedor_metas_historico
  FOR SELECT TO authenticated
  USING (auth.uid() = user_id OR public.has_role(auth.uid(), 'admin'));

CREATE POLICY "admin manage historico" ON public.vendedor_metas_historico
  FOR ALL TO authenticated
  USING (public.has_role(auth.uid(), 'admin'))
  WITH CHECK (public.has_role(auth.uid(), 'admin'));

-- 4) Snapshot mensal (upsert): consolida mês/ano dado
...
BEGIN
  ...
  LEFT JOIN public.vendedor_metas vm ON vm.user_id = ur.user_id
  LEFT JOIN (
    SELECT l.owner_id AS vid, count(*)::int AS qtd, coalesce(sum(l.estimated_value),0) AS valor
    FROM public.leads l
    WHERE l.stage='ganho'
      AND l.etapa_changed_at >= _p_start AND l.etapa_changed_at < _p_end
    GROUP BY l.owner_id
  ) g ON g.vid = ur.user_id
...
-- 5) placar_vendedores — bônus escalonado + dias_sem_proposta + pace
...
RETURNS TABLE(
  vendedor_id uuid, nome text, avatar_color text,
  ganhos_qtd bigint, ganhos_valor numeric,
  propostas_qtd bigint, conversao numeric, perdas_qtd bigint,
  leads_contatados bigint,
  tempo_medio_primeira_resposta_min numeric,
  slas_estourados bigint,
  carteira_45_60 bigint, carteira_60_mais bigint,
  pos_venda_no_prazo_pct numeric,
  meta_valor numeric, meta_pct numeric, meta_batida boolean,
  meta_faixa int, meta_pace_esperado_pct numeric,
  dias_sem_proposta int, dias_sem_proposta_limite int,
  score numeric, score_periodo_anterior numeric,
  posicao integer
)
...
DECLARE
  ...
  _dia int; _dias_uteis_total int; _dias_uteis_passados int;
  _pace numeric := NULL;
BEGIN
  ...
  -- Pace: fração de dias úteis já passados no mês (seg–sex, aproximação sem feriados)
  IF _is_mes THEN
    SELECT count(*) INTO _dias_uteis_total
      FROM generate_series(_p_start::date, (_p_end - interval '1 day')::date, '1 day') d
      WHERE extract(dow FROM d) BETWEEN 1 AND 5;
    SELECT count(*) INTO _dias_uteis_passados
      FROM generate_series(_p_start::date, LEAST(_sp::date, (_p_end - interval '1 day')::date), '1 day') d
      WHERE extract(dow FROM d) BETWEEN 1 AND 5;
    IF _dias_uteis_total > 0 THEN
      _pace := round((_dias_uteis_passados::numeric / _dias_uteis_total) * 100, 1);
    END IF;
  END IF;
  ...
  ult_proposta AS (
    SELECT l.owner_id AS vid, max(l.proposta_enviada_at) AS ult FROM public.leads l
    WHERE l.proposta_enviada_at IS NOT NULL GROUP BY l.owner_id
  ),
  perdas AS (
    SELECT l.owner_id AS vid, count(*)::bigint AS qtd FROM public.leads l
  ...
  agg AS (
    SELECT
      ...
      CASE
        WHEN NOT _is_mes OR coalesce(vm.meta_valor_mensal,0)=0 THEN 0
        WHEN coalesce(g.valor,0) >= vm.meta_valor_mensal * 1.20 THEN 120
        WHEN coalesce(g.valor,0) >= vm.meta_valor_mensal         THEN 100
        WHEN coalesce(g.valor,0) >= vm.meta_valor_mensal * 0.80  THEN 80
        WHEN coalesce(g.valor,0) >= vm.meta_valor_mensal * 0.50  THEN 50
        ELSE 0
      END AS meta_faixa,
      CASE
        WHEN up.ult IS NULL THEN NULL
        ELSE greatest(0, extract(day FROM (_now - up.ult))::int)
      END AS dias_sem_proposta,
      (
      ...
      + CASE
          WHEN NOT _is_mes OR coalesce(vm.meta_valor_mensal,0)=0 THEN 0
          WHEN coalesce(g.valor,0) >= vm.meta_valor_mensal * 1.20 THEN _cfg.placar_peso_meta_batida * 1.5
          WHEN coalesce(g.valor,0) >= vm.meta_valor_mensal         THEN _cfg.placar_peso_meta_batida * 1.0
          WHEN coalesce(g.valor,0) >= vm.meta_valor_mensal * 0.80  THEN _cfg.placar_peso_meta_batida * 0.5
          WHEN coalesce(g.valor,0) >= vm.meta_valor_mensal * 0.50  THEN _cfg.placar_peso_meta_batida * 0.25
          ELSE 0
        END
      )::numeric AS score,
      (
      ...
      + CASE
          WHEN NOT _is_mes OR coalesce(vm.meta_valor_mensal,0)=0 THEN 0
          WHEN coalesce(gp.valor,0) >= vm.meta_valor_mensal * 1.20 THEN _cfg.placar_peso_meta_batida * 1.5
          WHEN coalesce(gp.valor,0) >= vm.meta_valor_mensal         THEN _cfg.placar_peso_meta_batida * 1.0
          WHEN coalesce(gp.valor,0) >= vm.meta_valor_mensal * 0.80  THEN _cfg.placar_peso_meta_batida * 0.5
          WHEN coalesce(gp.valor,0) >= vm.meta_valor_mensal * 0.50  THEN _cfg.placar_peso_meta_batida * 0.25
          ELSE 0
        END
    ...
    LEFT JOIN ult_proposta up     ON up.vid = v.vid
  ...
  SELECT
    ...
    a.meta_faixa, _pace AS meta_pace_esperado_pct,
    a.dias_sem_proposta, _cfg.placar_dias_sem_proposta_limite AS dias_sem_proposta_limite,
