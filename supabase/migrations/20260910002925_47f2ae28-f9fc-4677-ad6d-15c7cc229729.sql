...
DECLARE
  ...
  _dias_uteis_total int; _dias_uteis_passados int;
  _pace numeric := NULL;
BEGIN
  ...
  IF _is_mes THEN
    SELECT count(*) INTO _dias_uteis_total
      FROM generate_series(
        (_p_start AT TIME ZONE 'America/Sao_Paulo')::date,
        ((_p_end - interval '1 day') AT TIME ZONE 'America/Sao_Paulo')::date,
        '1 day'
      ) d
    ...
    SELECT count(*) INTO _dias_uteis_passados
      FROM generate_series(
        (_p_start AT TIME ZONE 'America/Sao_Paulo')::date,
        LEAST(_sp::date, ((_p_end - interval '1 day') AT TIME ZONE 'America/Sao_Paulo')::date),
        '1 day'
      ) d
      WHERE extract(dow FROM d) BETWEEN 1 AND 5;
  ...
  slas AS (
    -- Um desconto por cliente/lead no período (antes: um por evento repetido).
    SELECT xl.vendedor_id AS vid,
           count(DISTINCT coalesce(xl.lead_id, xl.cliente_id, xl.id))::bigint AS qtd
    FROM public.xerife_log xl
    WHERE xl.regra ILIKE '%_escalado' AND xl.created_at >= _p_start AND xl.created_at < _p_end
